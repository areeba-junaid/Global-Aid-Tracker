      ...
      <div className="border border-gray-500 rounded w-full max-w-md-40">
        ...
        <div className="p-4 font-semibold font-sans">
          ...
          <div className="rounded mt-4 flex justify-center items-center">
            <p className="mr-20" style={{ fontSize: "20px" }}>Proposals: {offer.requestedBy.length}</p>
            <p className="ml-20" style={{ fontSize: "20px" }}>Accepted: {offer.acceptedDonee.length}</p>
          </div>

          {/* Combined container for Donee AIDNAME, Country, and Phone Number with icons */}
          <div className="rounded mt-4 flex justify-center items-center">
            <Person className="mr-1" /> <p className="mr-6">{offer.donor.name}</p>
            <Public className="mx-1" /> <p className="mx-">{offer.donor.country}</p>
            <Phone className="ml-10" /> <p className="ml">{offer.donor.phone}</p>
          </div>
        </div>
      </div>
    </div>
